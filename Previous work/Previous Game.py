...
class Knight(player):
    ...
    def fight(self):
        ...
        while ehealth>0 or self.health>0:
            multiplier=1
            luck=randint(0,100)/100
            edamage_done=edamage-(self.defence//5)
            if luck>=0.85:
                print("Your elite training allows you to hit for double damage this turn")
                multiplier=1.5
            if edamage<=0:
                self.health=self.health
            elif edamage>0:
                ...
                self.health=self.health-edamage_done
                damage_done=randint((self.damage-5),(self.damage+5))
                ehealth=ehealth-(damage_done*multiplier)
            if self.health<=0:
                if self.health_pots>0:
                    self.heal()
                    print("You make one last stand against the",current_enemy)
...
class Merchant(player):
    ...
    def fight(self):
        ...
        index=randint(0,len(enemy_array[0])-1)
        current_enemy=enemy_array[0][index]
        choice=input("Do you want to pay the monster to leave you alone")
        if choice.lower()=="yes":
            if self.gold-15<0:
                self.gold=self.gold-15
                print("You paid the, ",current_enemy," 15 gold")
                playing(self)
            else:
                print("Your measly amount of gold was not sufficient")
        if self.level<2:
            ehealth=enemy_array[1][index]
            edamage=enemy_array[2][index]
        else:
...
class Healer(player):
    ...
    def fight(self):
        enemy_array=[["skeleton","zombie","slime","ghost","goblin","rat"],
             [randint(15,45),randint(5,15),randint(5,10),randint(1,5),randint(20,40),randint(1,3)],      #enemy health
             [15,10,5,1,13,2],                                                                           #enemy damage
             [randint(15,25),randint(3,7),randint(3,5),randint(1,5),randint(12,22),randint(1,3)],        #gold dropped
             [randint(45,135),randint(15,45),randint(10,30),randint(5,15),randint(40,120),randint(3,9)]] #xp given
        index=randint(0,len(enemy_array[0])-1)
        current_enemy=enemy_array[0][index]
        if self.level<2:
            ehealth=enemy_array[1][index]
        ...
        while ehealth>0 or self.health>0:
            ...
            print(luck)
            if luck>=0.85:
                print("Your healing senses allow you to dodge the enemies attack this turn")
                miss=0
            edamage_done=edamage-(self.defence//5)
            if edamage<=0:
                self.health=self.health
                ...
                sleep(1.5)
                print("You have",self.health,"health and the enemy has",ehealth,"health")
                self.health=self.health-(edamage_done*miss)
                damage_done=randint((self.damage-5),(self.damage+5))
                ehealth=ehealth-damage_done
            if self.health<=0:
                if self.health_pots>0:
...
def playing(self):
    if character.xp>character.xp_needed:
        character.levelling()
...
if choice.lower() == "knight":
    print("You have chosen to become a fallen Knight: Wounded yet Noble")
    character = Knight(150,75,1,15,0,0,"Armour of the Knightly warriors","Sword of the fallen knight",0,5,0,0,0)
    idle(character)
elif choice.lower() == "merchant":
    print("You have chosen to be become a wealthy merchant of the guilds: Rich and Vain")
    character = Merchant(75,0,0,0,0,randint(50,150),"Garbs of Merchant guilds","No weapon",0,3,0,0,randint(5,15))
    idle(character)
elif choice.lower() == "healer":
    print("You have chosen to become a healer of the clerical class: Regal yet Tough")
